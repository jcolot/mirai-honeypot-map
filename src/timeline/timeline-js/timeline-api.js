...
(function() {
    ...
    var loadMe = function() {
        ...
        window.Timeline.DateTime = window.Timeline.DateTime; // for backward compatibility
        window.Timeline.includeJavascriptFile = function(doc, url, onerror, charset) {
            onerror = onerror || "";
            if (doc.body == null) {
                try {
        ...
        window.Timeline.includeJavascriptFiles = function(doc, urlPrefix, filenames) {
            for (var i = 0; i < filenames.length; i++) {
                Timeline.includeJavascriptFile(doc, urlPrefix + filenames[i]);
            }
            Timeline.loadingScriptsCount += filenames.length;
            Timeline.includeJavascriptFile(doc, Timeline.urlPrefix + "scripts/signal.js?" + filenames.length);
        };
        window.Timeline.includeCssFile = function(doc, url) {
            if (doc.body == null) {
                try {
       ...
       window.Timeline.includeCssFiles = function(doc, urlPrefix, filenames) {
            for (var i = 0; i < filenames.length; i++) {
                Timeline.includeCssFile(doc, urlPrefix + filenames[i]);
            }
        }; 
        ...
        try {
            ...
            var includeJavascriptFiles = function(urlPrefix, filenames) {
                Timeline.includeJavascriptFiles(document, urlPrefix, filenames);
            }
            var includeCssFiles = function(urlPrefix, filenames) {
                Timeline.includeCssFiles(document, urlPrefix, filenames);
            }
