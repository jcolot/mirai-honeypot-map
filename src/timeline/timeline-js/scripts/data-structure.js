 ...
 *
 * @constructor
 * @param {Array or Timeline.Set} [a] an initial collection
 */
Timeline.Set = function(a) {
    this._hash = {};
    this._count = 0;
    ...
    } else if (a instanceof Timeline.Set) {
        this.addSet(a);
    }
}
 ...
 */
Timeline.Set.prototype.add = function(o) {
    if (!(o in this._hash)) {
        this._hash[o] = true;
        this._count++;
        return true;
 ...
 *
 * @param {Timeline.Set} set the set of elements to add
 */
Timeline.Set.prototype.addSet = function(set) {
 ...
 *   false otherwise
 */
Timeline.Set.prototype.remove = function(o) {
    if (o in this._hash) {
        delete this._hash[o];
        this._count--;
        return true;
 ...
 * given set
 *
 * @param {Timeline.Set} set the set of elements to remove
 */
Timeline.Set.prototype.removeSet = function(set) {
    for (var o in set._hash) {
        this.remove(o);
    }
}
 ...
 *
 * @param {Timeline.Set} set the set to intersect
 */
Timeline.Set.prototype.retainSet = function(set) {
 ...
 *
 * @param {Timeline.Set} o the object to test for
 * @return {Boolean} true if the object is present, false otherwise
 */
Timeline.Set.prototype.contains = function(o) {
 ...
 */
Timeline.Set.prototype.size = function() {
    return this._count;
}
 ...
 */
Timeline.Set.prototype.toArray = function() {
    var a = [];
    for (var o in this._hash) {
        a.push(o);
    }
 ...
 */
Timeline.Set.prototype.visit = function(f) {
    for (var o in this._hash) {
        if (f(o) == true) {
            break;
        }
...
Timeline.EventIndex.prototype.getEvent = function(id) {
...
Timeline.EventIndex.prototype.getReverseIterator = function(startDate, endDate) {
    if (!this._indexed) {
        this._index();
    }
    return new Timeline.EventIndex._ReverseIterator(this._events, startDate, endDate, this._unit);
};
...
Timeline.EventIndex._ReverseIterator = function(events, startDate, endDate, unit) {
    this._events = events;
    this._startDate = startDate;
    this._endDate = endDate;
    this._unit = unit;
...
Timeline.EventIndex._ReverseIterator.prototype = {
    hasNext: function() { return this._hasNext; },
    next: function() {
        if (this._hasNext) {
            var next = this._next;
