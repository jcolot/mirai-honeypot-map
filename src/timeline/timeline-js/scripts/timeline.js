...
Timeline.display_version = Timeline.version;
...
Timeline.createBandInfo = function(params) {
    ...
    var ether = new Timeline.LinearEther({ 
        ...
        interval:           ("zoomIndex" in params) ?
                                Timeline.DateTime.gregorianUnitLengths[params.zoomSteps[params.zoomIndex].unit]:
                                Timeline.DateTime.gregorianUnitLengths[params.intervalUnit],
        pixelsPerInterval:  ("zoomIndex" in params) ? 
                                params.zoomSteps[params.zoomIndex].pixelsPerInterval:params.intervalPixels,
        theme:              theme
    });
    var etherPainter = new Timeline.GregorianEtherPainter({
        unit:       ("zoomIndex" in params) ?
                        params.zoomSteps[params.zoomIndex].unit:params.intervalUnit,
...
Timeline.loadXML = function(url, f) {
    var fError = function(xmlhttp, statusText) {
    ...
    jQuery.get(url).complete(fDone).error(fError);
};
...
Timeline._Impl.prototype.loadXML = function(url, f) {
    ...
    window.setTimeout(function() { $.get(url).complete(fDone).error(fError); }, 0);
};
...
Timeline._Impl.prototype.loadJSON = function(url, f) {
    ...
    this.showLoadingMessage();
    window.setTimeout(function() { $.get(url).error(fError).done(fDone); }, 0);
};
...
Timeline._Impl.prototype._initialize = function() {
     ...
     */
    var elmtCopyright = document.createElement("img");
    elmtCopyright.src = (Timeline.urlPrefix + (this.isHorizontal() ? "images/copyright-vertical.png" : "images/copyright.png"));
    elmtCopyright.className = "timeline-copyright";
    elmtCopyright.title = "SIMILE Timeline - http://www.simile-widgets.org/";
    $(elmtCopyright).click(function() { window.location = "http://www.simile-widgets.org/"; });
