...
Timeline._Band = function(timeline, bandInfo, index) {
    ...
    this._friction = ("friction" in bandInfo) ? bandInfo.friction : 0.95;
    this._dragSpeed = 0;
    ...
    this._listeners["orthogonalscroll"] = [];
    this._listeners["dragstart"] = [];
    this._listeners["drag"] = [];
    ...
    jQuery(this._div).on("vmousedown", function(evt){me._onMouseDown(evt)});
    jQuery(this._div).dblclick(function(evt){me._onDblClick(evt)});
    jQuery(document.body).on("vmouseup", function(evt){me._onMouseUp(evt)});
    jQuery(document.body).on("vmousemove", function(evt){me._onMouseMove(evt)});
    jQuery(document.body).on("vmouseout", function(evt){me._onMouseOut(evt)});
...
//    SimileAjax.DOM.registerEventWithObject(document.body, "mouseout", this, "_onMouseOut");
    ...
    if (mouseWheel === 'zoom' || mouseWheel === 'scroll' || this._zoomSteps) {
        ...
        jQuery(this._div).mousewheel(function(evt) {me._onMouseScroll(evt, me._div)});
    ...
    if (this._supportsOrthogonalScrolling) {
        ...
        scrollbarThumb.style.cursor = "-webkit-grab";
...
Timeline._Band.prototype.addListener = function(type, listener) {
   // this._listeners[type].push(listener);
   $(this).bind(type, listener);
};

Timeline._Band.prototype.removeListener = function(type, listener) {
...
Timeline._Band.prototype.setSyncWithBand = function(band, highlight) {
    if (this._syncWithBand) {
        this._syncWithBand.removeListener("scroll", this._syncWithBandHandler);
        this._syncWithBand.removeListener("orthogonalscroll", 
                this._syncWithBandOrthogonalScrollHandler);
    }
    
    this._syncWithBand = band;
    this._syncWithBand.addListener("scroll", this._syncWithBandHandler);
...
Timeline._Band.prototype.createLayerDiv = function(zIndex, className) {
    ...
    innerDiv.style.cursor = "-webkit-grab";
...
Timeline._Band.prototype.zoom = function(zoomIn, x, y, target) {
    ...
    // if zoom programatically, zoom at center
    if (x == undefined){
        x = -this._viewOffset + this._viewLength / 2
    }
...
Timeline._Band.prototype._momentum = function() {
    var me = this;
    var friction = this._friction;
    me._dragSpeed = me._dragSpeed * friction;

    var run = function(){
        me._moveEther(Math.round(-me._dragSpeed*20));
        if (Math.abs(me._dragSpeed * 20) > 1) {
            me._momentum();
        } else {
            me._dragSpeed = 0;
        };
...
Timeline._Band.prototype._onMouseDown = function(evt) {
    for (var i = 0; i < this._timeline._bands.length; i++)
        this._timeline._bands[i]._dragSpeed = 0;

    if (!this._dragging) {
        this.closeBubble();
        this._fireOnEvent("dragstart");
        this._dragging = true;
        this._dragX = evt.clientX;
        this._dragY = evt.clientY;
        this._div.style.cursor = "-webkit-grabbing";  
        return this._cancelEvent(evt);
    }
};

Timeline._Band.prototype._onMouseMove = function(evt) {
    if (this._dragging || this._orthogonalDragging) {
        var diffX = evt.clientX - this._dragX;
        var diffY = evt.clientY - this._dragY;
        var diffTime = this._now - (this._now = new Date().getTime());
        if (this._timeline.isHorizontal()) {
            this._dragSpeed = diffX / diffTime; 
        } else {
            this._dragSpeed = diffY / diffTime; 
        }
 
        this._dragX = evt.clientX;
        this._dragY = evt.clientY;
        this._fireOnEvent("drag");
    }
...
Timeline._Band.prototype._onMouseUp = function(evt) {
    if (this._dragging) {
        this._dragging = false;
        this._fireOnEvent("dragend"); 
    } else if (this._orthogonalDragging) {
        this._orthogonalDragging = false;
    } else {
        return;
    }
    this._div.style.cursor = "-webkit-grab"; 
    if(Math.abs(this._dragSpeed)>0.2) {
        this._momentum();
    }
...
Timeline._Band.prototype._onMouseScroll = function(evt, innerFrame) {
  var now = new Date();
  now = now.getTime();
...
Timeline._Band.prototype._onDblClick = function(evt) {
    var coords = SimileAjax.DOM.getEventRelativeCoordinates(evt, this._div);
...
Timeline._Band.prototype._autoScroll = function(distance, f) {
    var b = this;
    var a = Timeline.Graphics.createAnimation(
        function(abs, diff) {
            b._moveEther(diff);
        }, 
        0, 
...
Timeline._Band.prototype._moveEther = function(shift, orthogonalShift) {
    ...
    this._changingMod = (this._changingMod + 1) % 10;
    if (this._changingMod == 0) this._onChanging();
}

Timeline._Band.prototype._changingMod = 0;

Timeline._Band.prototype._onChanging = function() {
    this._changing = true;

    this._fireOnEvent("scroll");
...
Timeline._Band.prototype._fireOnEvent = function(type) {
   // for (var i = 0; i < this._listeners[type].length; i++) {
...
Timeline._Band.prototype._paintEvents = function() {
    ...
    this._showScrollbar();
    this._fireOnEvent("orthogonalscroll");
};
...
Timeline._Band.prototype._bounceBack = function(f) {
    ...
    if (target != this._viewOrthogonalOffset) {
        var self = this;
        Timeline.Graphics.createAnimation(
            function(abs, diff) {
                self._viewOrthogonalOffset = abs;
                self._eventPainter.softPaint();
                self._showScrollbar();
                self._fireOnEvent("orthogonalscroll");
            }, 
...
// JC33 - Center events, at the minimum zoomlevel to display all events 
Timeline._Band.prototype.centerOnEvents = function(animate) {
    var me = this;
    var earliestEvent = this._eventSource.getEarliestDate();
    var latestEvent = this._eventSource.getLatestDate();
    if (!(latestEvent && earliestEvent)) {
       return; 
    };
    var eventsTimespan = latestEvent.getTime() - earliestEvent.getTime();
    var eventsCenter = new Date(earliestEvent.getTime() + eventsTimespan / 2);
    for (var i = 0 ; i < this._zoomSteps.length ; i++) {
         
        var bandTimespan = this.getViewLength() / 
            this._zoomSteps[i].pixelsPerInterval * 
            Timeline.DateTime.gregorianUnitLengths[this._zoomSteps[i].unit];
        
        if (bandTimespan >= eventsTimespan ) {
            this.zoom(i); 
            this.paint();
            if (animate) {
                this.scrollToCenter(eventsCenter);
            } else { 
                this.setCenterVisibleDate(eventsCenter);
            }
            return; 
        }
    }
