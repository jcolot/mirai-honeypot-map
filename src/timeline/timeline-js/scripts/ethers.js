...
Timeline.LinearEther.prototype.zoom = function(zoomIn) {
  ...
  } else if (typeof zoomIn === "number") { 
      if (zoomIn >= 0 && zoomIn < this._band._zoomSteps.length) {
        newZoomIndex = zoomIn;
      }
  } else {
      alert("zoomIn should be either a boolean or a number");
  }

  this._band._zoomIndex = newZoomIndex;  
  this._interval = 
      Timeline.DateTime.gregorianUnitLengths[this._band._zoomSteps[newZoomIndex].unit];
...
Timeline.HotZoneEther.prototype.zoom = function(zoomIn) {
  ...
  var currentZoomIndex = this._band._zoomIndex;
  var newZoomIndex = currentZoomIndex;
  // JC33 changed the function to accept a number zoomindex instead of 
  // boolean
  if (typeof(zoomIn) == "boolean") {
      if (zoomIn && (currentZoomIndex > 0)) {
          newZoomIndex = currentZoomIndex - 1;
      ...
      if (zoomIn >= 0 && zoomIn < this._band._zoomSteps.length) {
        ...
        alert("zoom parameter should be either number or boolean");
  }
