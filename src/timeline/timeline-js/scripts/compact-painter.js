...
Timeline.CompactEventPainter.prototype.paint = function() {
    ...
    var clusterEvents = "clusterEvents" in this._params;
    ...
    if (collapseConcurrentPreciseInstantEvents || stackConcurrentPreciseInstantEvents) {
        ...
        while (iterator.hasNext()) {
            ...
            if (filterMatcher(evt)) {
                if (!evt.isInstant() || evt.isImprecise()) {
                    ...
                    this._bufferedEvents[this._bufferedEvents.length - 1].push(evt);
                } else {
                    this._bufferedEvents.push([ evt ]);
        ...
        for (var i = 0; i < this._bufferedEvents.length; i++) {
            var compositeEvents = this._bufferedEvents[i];
    ...
    } else if (clusterEvents) {
        this._bufferedEvents = [];
        var firstEventInCluster = null;
            while (iterator.hasNext()) {
                var evt = iterator.next();
                ...
                if (filterMatcher(evt)) {
                    if (!evt.isInstant() || evt.isImprecise()) {
                        ...
                        var p1 = this._band.dateToPixelOffset(previousInstantEvent.getStart());
                        var p2 = this._band.dateToPixelOffset(evt.getStart());

                        if(Math.abs(p2 - p1) < 34) {
                            this._bufferedEvents[this._bufferedEvents.length - 1].push(evt);
                            previousInstantEvent = evt;
                            }
                        else {
                        ...
                        this._bufferedEvents.push([ evt ]);
                        firstEventInCluster = evt;
                        previousInstantEvent = evt;
                    }
                }
            }
            for (var i = 0; i < this._bufferedEvents.length; i++) {
                var clusteredEvents = this._bufferedEvents[i];
                if (clusteredEvents.length == 1) {
                    this.paintEvent(clusteredEvents[0], metrics, this._params.theme, highlightMatcher(evt)); 
                } else {
                    var match = -1;
                    for (var j = 0; match < 0 && j < clusteredEvents.length; j++) {
                        match = highlightMatcher(clusteredEvents[j]);
                    }
                    this.paintClusteredInstantEvents(clusteredEvents, metrics, this._params.theme, match);
                }
            }
        } else {
        while (iterator.hasNext()) {
...
Timeline.CompactEventPainter.prototype._computeMetrics = function() {
    ...
    var metrics = {
        ...
        defaultIconAlign:       eventTheme.instant.iconAlign,
        ...
        customIconAlign:        "iconAlign" in this._params ? this._params.iconAlign : eventTheme.instant.iconAlign,
    ...
    if ("compositeIcon" in this._params) {
        ...
        metrics.compositeIconAlign = this._params.compositeIconAlign || metrics.customIconAlign;
    } else {
        ...
        metrics.compositeIconAlign =  metrics.defaultIconAlign;
    }
    ...
    metrics.defaultStackIconAlign = ("stackConcurrentPreciseInstantEvents" in this._params && "iconAlign" in this._params.stackConcurrentPreciseInstantEvents) ?
        this._params.stackConcurrentPreciseInstantEvents.iconAlign : metrics.defaultIconAlign;
...
Timeline.CompactEventPainter.prototype._prepareForPainting = function() {
    ...
    if (this._backLayer == null) {
        ...
        this._frc = Timeline.Graphics.getFontRenderingContext(eventLabelPrototype);
    }
    this._frc.update();
    this._tracks = [];
...
Timeline.CompactEventPainter.prototype.paintPreciseInstantEvent = function(evt, metrics, theme, highlightIndex) {
    ...
    if (iconData.url == null) {
        ...
        iconData.iconAlign = metrics.defaultIconAlign;
        ...
        iconData.iconAlign = metrics.customIconAlign;
    }
    ...
    $(result.iconElmtData.elmt).bind("mousedown", clickHandler);
    $(result.labelElmtData.elmt).bind("mousedown", clickHandler);
...
Timeline.CompactEventPainter.prototype.paintCompositePreciseInstantEvents = function(events, metrics, theme, highlightIndex) {
    ...
    var iconData = {
        ...
        iconAlign: metrics.compositeIconAlign,
...
Timeline.CompactEventPainter.prototype.paintClusteredInstantEvents = function(events, metrics, theme, highlightIndex) {
    var evt = events[0];
//    this.paintPreciseInstantEvent(evt, metrics, theme, highlightIndex);
...
Timeline.CompactEventPainter.prototype.paintStackedPreciseInstantEvents = function(events, metrics, theme, highlightIndex) {
    ...
    var getIconData = function(evt) {
        ...
        if (iconData.url == null) {
            ...
            iconData.iconAlign = metrics.defaultStackIconAlign;
            iconData.className = "timeline-event-icon-stack timeline-event-icon-default";
        } else {
            iconData.width = evt.getProperty("iconWidth") || metrics.customIconWidth;
            iconData.height = evt.getProperty("iconHeight") || metrics.customIconHeight;
            iconData.iconAlign = evt.getProperty("iconAlign") || metrics.customIconAlign;
    ...
    var paintEvent = function(index) {
        ...
        var img = Timeline.Graphics.createTranslucentImage(record.iconData.url);
        ...
        $(iconDiv).bind("mousedown", clickHandler);
        $(labelElmtData.elmt).bind("mousedown", clickHandler);
    ...
    if (showMoreMessage) {
        ...
        $(moreMessageLabelElmtData.elmt).bind("mousedown", moreMessageClickHandler);
...
Timeline.CompactEventPainter.prototype.paintImpreciseInstantEvent = function(evt, metrics, theme, highlightIndex) {
    ...
    $(result.labelElmtData.elmt).bind("mousedown", clickHandler);
    $(result.impreciseTapeElmtData.elmt).bind("mousedown", clickHandler);
...
Timeline.CompactEventPainter.prototype.paintPreciseDurationEvent = function(evt, metrics, theme, highlightIndex) {
    ...
    $(result.labelElmtData.elmt).bind("mousedown", clickHandler);
    $(result.tapeElmtData.elmt).bind("mousedown", clickHandler);
...
Timeline.CompactEventPainter.prototype._paintEventTape = function(
    ...
    Timeline.Graphics.setOpacity(tapeDiv, opacity);
...
Timeline.CompactEventPainter.prototype._showBubble = function(x, y, evts) {
   ...
   // Timeline.Graphics.createBubbleForContentAndPoint(div, x, y, this._params.theme.event.bubble.width);
};
