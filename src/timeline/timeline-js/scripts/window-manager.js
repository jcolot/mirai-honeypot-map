...
Timeline.WindowManager.initialize = function() {
    ...
    SimileAjax.DOM.registerEvent(document.body, "mousedown", Timeline.WindowManager._onBodyMouseDown);
    SimileAjax.DOM.registerEvent(document.body, "mousemove", Timeline.WindowManager._onBodyMouseMove);
    SimileAjax.DOM.registerEvent(document.body, "mouseup",   Timeline.WindowManager._onBodyMouseUp);
    SimileAjax.DOM.registerEvent(document, "keydown",       Timeline.WindowManager._onBodyKeyDown);
    SimileAjax.DOM.registerEvent(document, "keyup",         Timeline.WindowManager._onBodyKeyUp);
            ...
            if (Math.abs(diffX) > 5 || Math.abs(diffY) > 5) {
                try {
                    ...
                    if ("ghost" in callback && callback.ghost) {
                        ...
                        Timeline.Graphics.setOpacity(ghostElmt, 50);
                    ...
                    if ("droppable" in callback && callback.droppable) {
                        ...
                        if (target != Timeline.WindowManager._potentialDropTarget) {
                            ...
                            if (droppable) {
                                ...
                                Timeline.Graphics.setOpacity(highlight, 30);
